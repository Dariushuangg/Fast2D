...
class Shader : public GShader {
public:
    Shader(const GBitmap& ShaderBM, const GMatrix& localInverse, GShader::TileMode tileMode) : 
    ShaderBM(ShaderBM), 
        ...
        for (int j = 0; j < count; j ++) {
            ...
            switch(mode) {
                ...
                case GShader::kRepeat: {
                    ...
                    assert(iy < ShaderBM.height()); //!! might be wrong
                    break;
                }
...
std::unique_ptr<GShader> GCreateBitmapShader(const GBitmap& ShaderBM,
 ...
 GShader::TileMode tileMode) {
    return std::unique_ptr<GShader>(new Shader(ShaderBM, localInverse, tileMode));
}
